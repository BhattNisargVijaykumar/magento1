<?php
class Nb_Vendor_Adminhtml_vendorController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction()
	{
		...
		if ($id) 
		{
			...
			foreach($postData as $key => $value)
			{
				...
				$dataName = $model->getResource()->getReadConnection()->fetchAll("SELECT * FROM `eav_attribute` WHERE `attribute_code` = '$key' AND `entity_type_id` = 9");
